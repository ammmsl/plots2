...
| §9 Content Additions Required | Status column added. Items that have been addressed in subsequently created documents are marked as resolved. The two items that remain outstanding are flagged. |
| §10 Document Relationships | Expanded to include `feature_layers.md`, `setup_optimisation_engine.md`, and `agent_access_spec.md`, all of which now draw from or relate to this document. |
...
The following additions were identified in v1.0 as needed to improve the interplay with the guide. Status reflects updates made in companion document revisions.

### `setup_tools_reference.md` additions

| Addition | What it enables | Status |
|---|---|---|
| `risk_if_wrong` per tool | Populates `FixStep.warning` — the user needs to know what the risk of misusing each tool is | ✅ **Resolved in v1.1** — added to all measurement tools (§1.1–§1.4) |
| `learning_value` rating per tool | Informs the learnable skill inventory — which tools are worth buying and developing vs. which are specialist-only | ✅ **Resolved in v1.1** — added to all measurement tools (§1.1–§1.4) |
| `cost_range` per tool | Populates `SetupTool.cost_range` — currently implied but not stated explicitly | ✅ **Resolved in v1.1** — added to all measurement tools (§1.1–§1.4) |

> ⚠️ **Outstanding:** `risk_if_wrong`, `learning_value`, and `cost_range` have been added to §1.1–§1.4 (measurement tools) in v1.1. Sections §2–§5 (adjustment, nut, fret, and general tools) still need these columns added in a future revision.

### `guitar_setup_parameters_v3.md` additions

| Addition | What it enables | Status |
|---|---|---|
| Plain-language symptom description per problem (alongside the formal parameter diagnostic) | Reduces translation work in the guide — the formal description and the plain-language version would be co-located | ⏳ **Outstanding** — not yet added; the guide currently maintains its own plain-language descriptions in the ProblemRecord schema |
| `rod_state` to `rod_adjustment` mapping table for common rod types | Populates `FixStep` content for truss rod skill pathway more precisely | ⏳ **Outstanding** |

### `string_tension_database.md` additions

| Addition | What it enables | Status |
|---|---|---|
| Note that `§6.3` (compliance coefficient model) is superseded by v3.0 flat-neck datum approach | Removes a stale cross-reference flagged in v3.0 §7 | ✅ **Resolved in v1.1** — §6.3 deprecated with full supersession notice |
| Plain-language summary of the tension formula for each tuning preset | Enables guide to surface "in Drop C, your low string produces about X lbs of tension" without requiring the user to run the formula | ⏳ **Outstanding** — the tension formula runs live in the simulation; preset summaries would need to be pre-computed and stored |
...
guitar_setup_parameters_v3.md §4 (Problem Index — P-01 through P-21, T-01 through T-05)

  ↓ draws tension physics and tuning data from
string_tension_database.md §2, §4, §6

  ↓ draws tool data from
setup_tools_reference.md §1–§8 (tool catalogue, risk_if_wrong, learning_value, cost_range)

  ↓ is described as the forward-facing layer in
feature_layers.md — Layer 1 (Symptom Guide) + Layer 2 (Skill Pathways)

  ↓ provides ProblemRecord schema and SkillTier definitions consumed by
setup_optimisation_engine.md §2.3 (reversibility and skill tier for candidate ranking)

  ↓ provides component specs consumed by
agent_access_spec.md §4.1 (ProblemRecord API schema derives from §4 of this document)
...
SkillPathway pages
TensionDashboard component (Layer 3 simulation entry)
  → Layer 4 optimisation entry: see setup_optimisation_engine.md §9.1

  ↓ which guides users toward
guitar_setup_parameters_v3.md (full simulation — ~10% of users via Layer 3)
setup_optimisation_engine.md (optimisation — ~5% of users via Layer 4)
string_tension_database.md (tension engine reference — ~2% of users)
agent_access_spec.md (agent/API access — developer audience)
```
...
*v1.1 changes: companion document list expanded; §9 Content Additions updated with resolution status; §10 Document Relationships expanded to all six sibling documents.*
*Companion documents: [`guitar_setup_parameters_v3.md`](./guitar_setup_parameters_v3.md) · [`string_tension_database.md`](./string_tension_database.md) · [`setup_tools_reference.md`](./setup_tools_reference.md) · [`feature_layers.md`](./feature_layers.md) · [`setup_optimisation_engine.md`](./setup_optimisation_engine.md) · [`agent_access_spec.md`](./agent_access_spec.md)*
