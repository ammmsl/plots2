...
**Version:** 1.1
**Companion documents:** [`setup_guide_architecture.md`](./setup_guide_architecture.md) · [`guitar_setup_parameters_v3.md`](./guitar_setup_parameters_v3.md) · [`string_tension_database.md`](./string_tension_database.md) · [`setup_tools_reference.md`](./setup_tools_reference.md) · [`setup_optimisation_engine.md`](./setup_optimisation_engine.md) · [`agent_access_spec.md`](./agent_access_spec.md)
...
| Section | Change |
|---|---|
| Header | Version bumped to 1.1; `setup_tools_reference.md` and `agent_access_spec.md` added to companion documents — both are part of the system this document describes. |
| §Layer 1 (Symptom Guide) | Cross-reference added to `setup_guide_architecture.md` SymptomPicker component spec. |
| §Layer 2 (Skill Pathways) | Cross-reference added to `setup_tools_reference.md` for the tool skill-building content. |
| §How the layers connect | Note added: the agent access layer (`agent_access_spec.md`) mirrors the same four-layer journey for programmatic consumers. |
| Footer | Version line and companion document list updated. |
...
> **Implementation reference:** The data structures that drive this layer — `SymptomEntry`, `ProblemRecord`, `SkillTier`, `OffRamp` — are fully specified in [`setup_guide_architecture.md §4`](./setup_guide_architecture.md#4-data-structures). The `SymptomPicker` and `ProblemCard` component specs are in [`setup_guide_architecture.md §7`](./setup_guide_architecture.md#7-component-specifications).
...
The skill pathways connect directly to the parameter model. Every step that involves measurement references a specific parameter from [`guitar_setup_parameters_v3.md`](./guitar_setup_parameters_v3.md) — `relief_bass`, `nut_slot_height[n]`, `saddle_height[1..6]`. The pathway doesn't expose those parameter names to the user by default, but it uses the same target values and the same diagnostic logic.

Tool selection and risk guidance for each pathway draws from [`setup_tools_reference.md`](./setup_tools_reference.md), which now includes `risk_if_wrong`, `learning_value`, and `cost_range` per tool — the three data points that determine what goes into the pathway's "practice advice," "common mistakes," and "what to buy first" sections. High `learning_value` tools (feeler gauge, straight edge, action gauge) are the ones Layer 2 teaches players to use with confidence.
...
**Agent access mirror.** The same four-layer journey is available to programmatic consumers via [`agent_access_spec.md`](./agent_access_spec.md). An agent can replicate Layer 1 (symptom query → problem record), Layer 2 (tool requirements and fix steps), Layer 3 (tension calculation), and Layer 4 (optimisation session) through the REST API or MCP server. The agent access layer is a read interface over the same data that drives this site — no inference is hosted separately.

---

*Document version 1.1*
*v1.1 changes: companion document list expanded; Layer 1 cross-reference to `setup_guide_architecture.md` added; Layer 2 cross-reference to `setup_tools_reference.md` added; agent access mirror note added to "How the layers connect".*
*Companion documents: [`setup_guide_architecture.md`](./setup_guide_architecture.md) · [`guitar_setup_parameters_v3.md`](./guitar_setup_parameters_v3.md) · [`string_tension_database.md`](./string_tension_database.md) · [`setup_tools_reference.md`](./setup_tools_reference.md) · [`setup_optimisation_engine.md`](./setup_optimisation_engine.md) · [`agent_access_spec.md`](./agent_access_spec.md)*
